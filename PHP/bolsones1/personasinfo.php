...
class cpersonas extends cTable {
	...
	function __construct() {
		...
		// nombre
		$this->nombre = new cField('personas', 'personas', 'x_nombre', 'nombre', '`nombre`', '`nombre`', 200, -1, FALSE, '`nombre`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nombre->Sortable = TRUE; // Allow sort
		$this->fields['nombre'] = &$this->nombre;

		// apellido
		$this->apellido = new cField('personas', 'personas', 'x_apellido', 'apellido', '`apellido`', '`apellido`', 200, -1, FALSE, '`apellido`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// padremadretutor
		$this->padremadretutor = new cField('personas', 'personas', 'x_padremadretutor', 'padremadretutor', '`padremadretutor`', '`padremadretutor`', 201, -1, FALSE, '`padremadretutor`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->padremadretutor->Sortable = TRUE; // Allow sort
		$this->padremadretutor->OptionCount = 3;
		$this->fields['padremadretutor'] = &$this->padremadretutor;

		// division
		$this->division = new cField('personas', 'personas', 'x_division', 'division', '`division`', '`division`', 200, -1, FALSE, '`division`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->division->Sortable = TRUE; // Allow sort
		$this->fields['division'] = &$this->division;

		// retirado
		$this->retirado = new cField('personas', 'personas', 'x_retirado', 'retirado', '`retirado`', '`retirado`', 16, -1, FALSE, '`retirado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->retirado->Sortable = TRUE; // Allow sort
		$this->retirado->OptionCount = 2;
		$this->retirado->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['retirado'] = &$this->retirado;

		// id_personas
		$this->id_personas = new cField('personas', 'personas', 'x_id_personas', 'id_personas', '`id_personas`', '`id_personas`', 3, -1, FALSE, '`id_personas`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_personas->Sortable = TRUE; // Allow sort
		$this->id_personas->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_personas'] = &$this->id_personas;
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->id_personas->setDbValue($conn->Insert_ID());
			$rs['id_personas'] = $this->id_personas->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_personas', $rs))
				ew_AddFilter($where, ew_QuotedName('id_personas', $this->DBID) . '=' . ew_QuotedValue($rs['id_personas'], $this->id_personas->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`id_personas` = @id_personas@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->id_personas->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_personas@", ew_AdjustSql($this->id_personas->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "id_personas:" . ew_VarToJson($this->id_personas->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_personas->CurrentValue)) {
			$sUrl .= "id_personas=" . urlencode($this->id_personas->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["id_personas"]))
				$arKeys[] = ew_StripSlashes($_POST["id_personas"]);
			elseif (isset($_GET["id_personas"]))
				$arKeys[] = ew_StripSlashes($_GET["id_personas"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_personas->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->padremadretutor->Exportable) $Doc->ExportCaption($this->padremadretutor);
					if ($this->division->Exportable) $Doc->ExportCaption($this->division);
					if ($this->retirado->Exportable) $Doc->ExportCaption($this->retirado);
					if ($this->id_personas->Exportable) $Doc->ExportCaption($this->id_personas);
				} else {
					if ($this->nombre->Exportable) $Doc->ExportCaption($this->nombre);
					if ($this->apellido->Exportable) $Doc->ExportCaption($this->apellido);
					if ($this->dni->Exportable) $Doc->ExportCaption($this->dni);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->padremadretutor->Exportable) $Doc->ExportField($this->padremadretutor);
						if ($this->division->Exportable) $Doc->ExportField($this->division);
						if ($this->retirado->Exportable) $Doc->ExportField($this->retirado);
						if ($this->id_personas->Exportable) $Doc->ExportField($this->id_personas);
					} else {
						if ($this->nombre->Exportable) $Doc->ExportField($this->nombre);
						if ($this->apellido->Exportable) $Doc->ExportField($this->apellido);
						if ($this->dni->Exportable) $Doc->ExportField($this->dni);
